...
public class OfficeToPdfFunction implements OutboundConnectorFunction, CherryConnector {
  ...
  public OfficeToPdfOutput execute(OutboundConnectorContext context) throws Exception {
    logger.info("OfficeToPdf.start");

    OfficeToPdfInput officeToPdfInput=null;
    try {
     officeToPdfInput = context.bindVariables(OfficeToPdfInput.class);
      ...
      throw new ConnectorException(BPMERROR_LOAD_FILE_ERROR,
          "Connector [OfficeToPdf] cannot bind variables "+e);

    }
    logger.info("OfficeToPdf.execute");
    ...
    try {
      ...
      logger.info("OfficeToPdf.saveToStorage[{}]",storageDefinitionDestination.getInformation());
      ...
      logger.error("OfficeToPdf: Conversion error file [" + (docSourceReference == null ?
          "" :
          FileVariableReference.getInformation(docSourceReference)) + "] " + e);
  ...
  private ByteArrayInputStream getByteArrayFromInput(InputStream inputStream) throws IOException {
